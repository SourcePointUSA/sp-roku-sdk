    ...
    if m.top.privacyManagerViewConsent <> invalid then
        ...
        if legIntCategories <> invalid then
            for each categoryId in legIntCategories
                ...
                if categories[categoryId] <> invalid and categories[categoryId].enabled = false and legIntCategories[categoryId].enabled = true then
                    consentedCat = {
                        consent: false,
                        "legInt": true,
    ...
    if m.top.privacyManagerViewConsent <> invalid then
        ...
        if legIntVendors <> invalid then
            for each vendorId in legIntVendors
                ' we already got all enabled categories
                ' we only missed disabled categories where leg int is enabled, so grab those
                if vendors[vendorId] <> invalid and vendors[vendorId].enabled = false and legIntVendors[vendorId].enabled = true then
                    consentedVendor = {
                        consent: false,
                        "legInt": true,

...
sub renderTabs()
    ...
    if m.components.text_purposes_header_text <> invalid then
        ...
        purposesUnderline.translation = [
            0,
            purposesHeader.boundingRect().height + tabPadding
        ]
    ...
    if m.components.text_data_categories_header <> invalid then
        ...
        categoriesUnderline.translation = [
            0,
            categoriesHeader.boundingRect().height + tabPadding
        ]
...
function isDisclosureOnly() as boolean
    if m.vendorData = invalid then
        ...
        return false
    else ' has disclosure only categories, make sure it has no toggleable types
        ret = true
        keys = [
            "consentCategories",
            "legIntCategories",
